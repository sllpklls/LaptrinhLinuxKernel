...
int matadd(int p, int q, int a[p*q], int b[p*q]) {
    ...
    for (i=0;i<p;i++){
        ...
        for (j=0;j<q;j++){
           // printk( "%-7d", *(c + (q * i) + j));
            sprintf(str + strlen(str), "%4d", *(c + (q * i) + j));
        }
        printk("%s\n", str);
    }
    return 0;
...
int matmul(int p, int q, int s, int a[p*q], int b[q*s], int c[p*s]) {
    ...
    for (i=0;i<p;i++){
        ...
        for (j=0;j<s;j++){
            // printk("%-7d", *(c + (s * i) + j));
            sprintf(str + strlen(str), "%4d", *(c + (s * i) + j));
        }
        printk("%s\n", str);
    }
    return 0;
...
int smallernuminmatrix(int p, int q, int a[p*q], int s) {
    ...
    printk("There are %d numbers smaller than %d in this matrix", count, s);
    return 0;
}
...
int numdivisibleinmatrix(int p, int q, int a[p*q], int s) {
    ...
    printk("There are %d numbers divisible by %d in this matrix", count, s);
    return 0;    
}
...
int primeofmat(int p, int q, int a[p*q]) {
    ...
    printk("There are %d prime numbers in this matrix", count);
    return 0;   
}
